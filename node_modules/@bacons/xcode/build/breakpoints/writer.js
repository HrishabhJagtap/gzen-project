...
exports.build = build;
function buildBucketAttributes(list) {
    const attrs = [];
...
function buildBreakpointProxy(proxy, depth) {
    const lines = [];
    const indent = getIndent(depth);
    lines.push(`${indent}<BreakpointProxy`);
    lines.push(`${indent}   BreakpointExtensionID = "${proxy.breakpointExtensionID}">`);
...
function buildBreakpointContent(content, depth) {
    const lines = [];
    const indent = getIndent(depth);
    const attrs = [];
    ...
    if (content.continueAfterRunningActions !== undefined) {
        attrs.push(`continueAfterRunningActions = "${boolToString(content.continueAfterRunningActions)}"`);
    }
    ...
    const hasChildren = (content.actions && content.actions.length > 0) ||
...
function buildBreakpointActionProxy(proxy, depth) {
    const lines = [];
    const indent = getIndent(depth);
...
function buildBreakpointActionContent(content, depth) {
    const lines = [];
    const indent = getIndent(depth);
    const attrs = [];
...
function buildBreakpointLocation(location, depth) {
    const lines = [];
    const indent = getIndent(depth);
    const attrs = [];
    ...
    if (location.shouldBeEnabled !== undefined) {
        attrs.push(`shouldBeEnabled = "${boolToString(location.shouldBeEnabled)}"`);
    }
    ...
    if (location.continueAfterRunningActions !== undefined) {
        attrs.push(`continueAfterRunningActions = "${boolToString(location.continueAfterRunningActions)}"`);
    }
