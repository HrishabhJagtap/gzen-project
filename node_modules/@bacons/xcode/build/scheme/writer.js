...
exports.build = build;
function buildSchemeAttributes(scheme) {
    const attrs = [];
...
function buildBuildAction(action, depth) {
    const lines = [];
    const indent = getIndent(depth);
    const attrs = [];
    if (action.parallelizeBuildables !== undefined) {
        attrs.push(`parallelizeBuildables = "${boolToString(action.parallelizeBuildables)}"`);
    }
    if (action.buildImplicitDependencies !== undefined) {
        attrs.push(`buildImplicitDependencies = "${boolToString(action.buildImplicitDependencies)}"`);
    }
    if (action.runPostActionsOnFailure !== undefined) {
        attrs.push(`runPostActionsOnFailure = "${boolToString(action.runPostActionsOnFailure)}"`);
    }
...
function buildBuildActionEntry(entry, depth) {
    const lines = [];
    const indent = getIndent(depth);
    const attrs = [];
...
function buildBuildableReference(ref, depth) {
    const lines = [];
    const indent = getIndent(depth);
    ...
    lines.push(`${indent}   ReferencedContainer = "${escapeXml(ref.referencedContainer)}">`);
    lines.push(`${indent}</BuildableReference>`);
    return lines;
}
function buildExecutionActions(containerName, actions, depth) {
    const lines = [];
    const indent = getIndent(depth);
...
function buildExecutionAction(action, depth) {
    const lines = [];
    const indent = getIndent(depth);
...
function buildActionContent(content, depth) {
    const lines = [];
    const indent = getIndent(depth);
    const attrs = [];
...
function buildTestAction(action, depth) {
    const lines = [];
    const indent = getIndent(depth);
    const attrs = [];
    ...
    if (action.selectedDebuggerIdentifier !== undefined) {
        attrs.push(`selectedDebuggerIdentifier = "${action.selectedDebuggerIdentifier}"`);
    }
    if (action.selectedLauncherIdentifier !== undefined) {
        attrs.push(`selectedLauncherIdentifier = "${action.selectedLauncherIdentifier}"`);
    }
    if (action.shouldUseLaunchSchemeArgsEnv !== undefined) {
        attrs.push(`shouldUseLaunchSchemeArgsEnv = "${boolToString(action.shouldUseLaunchSchemeArgsEnv)}"`);
    }
    if (action.shouldAutocreateTestPlan !== undefined) {
        attrs.push(`shouldAutocreateTestPlan = "${boolToString(action.shouldAutocreateTestPlan)}"`);
    }
    if (action.preferredScreenCaptureFormat !== undefined) {
        attrs.push(`preferredScreenCaptureFormat = "${action.preferredScreenCaptureFormat}"`);
    }
    if (action.codeCoverageEnabled !== undefined) {
        attrs.push(`codeCoverageEnabled = "${boolToString(action.codeCoverageEnabled)}"`);
    }
    if (action.onlyGenerateCoverageForSpecifiedTargets !== undefined) {
        attrs.push(`onlyGenerateCoverageForSpecifiedTargets = "${boolToString(action.onlyGenerateCoverageForSpecifiedTargets)}"`);
    }
...
function buildTestableReference(ref, depth) {
    const lines = [];
    const indent = getIndent(depth);
    const attrs = [];
    ...
    if (ref.useTestSelectionWhitelist !== undefined) {
        attrs.push(`useTestSelectionWhitelist = "${boolToString(ref.useTestSelectionWhitelist)}"`);
    }
...
function buildTestPlanReference(ref, depth) {
    const lines = [];
    const indent = getIndent(depth);
...
function buildLaunchAction(action, depth) {
    const lines = [];
    const indent = getIndent(depth);
    const attrs = [];
    if (action.buildConfiguration !== undefined) {
    ...
    if (action.appClipInvocationURLString !== undefined) {
        attrs.push(`appClipInvocationURLString = "${escapeXml(action.appClipInvocationURLString)}"`);
    }
    if (action.selectedDebuggerIdentifier !== undefined) {
        attrs.push(`selectedDebuggerIdentifier = "${action.selectedDebuggerIdentifier}"`);
    }
    ...
    if (action.askForAppToLaunch !== undefined) {
        attrs.push(`askForAppToLaunch = "${action.askForAppToLaunch ? "Yes" : "No"}"`);
    }
    if (action.useCustomWorkingDirectory !== undefined) {
        attrs.push(`useCustomWorkingDirectory = "${boolToString(action.useCustomWorkingDirectory)}"`);
    }
    ...
    if (action.ignoresPersistentStateOnLaunch !== undefined) {
        attrs.push(`ignoresPersistentStateOnLaunch = "${boolToString(action.ignoresPersistentStateOnLaunch)}"`);
    }
    if (action.debugDocumentVersioning !== undefined) {
        attrs.push(`debugDocumentVersioning = "${boolToString(action.debugDocumentVersioning)}"`);
    }
    ...
    if (action.allowLocationSimulation !== undefined) {
        attrs.push(`allowLocationSimulation = "${boolToString(action.allowLocationSimulation)}"`);
    }
    ...
    if (action.launchAutomaticallySubstyle !== undefined) {
        attrs.push(`launchAutomaticallySubstyle = "${action.launchAutomaticallySubstyle}"`);
    }
    ...
    if (action.buildableProductRunnable) {
        lines.push(...buildBuildableProductRunnable(action.buildableProductRunnable, depth + 1));
    }
    ...
    if (action.locationScenarioReference) {
        lines.push(...buildLocationScenarioReference(action.locationScenarioReference, depth + 1));
    }
    if (action.commandLineArguments && action.commandLineArguments.length > 0) {
        lines.push(...buildCommandLineArguments(action.commandLineArguments, depth + 1));
    }
    ...
    if (action.storeKitConfigurationFileReference) {
        lines.push(...buildStoreKitConfigurationFileReference(action.storeKitConfigurationFileReference, depth + 1));
    }
    lines.push(`${indent}</LaunchAction>`);
    return lines;
}
function buildBuildableProductRunnable(runnable, depth) {
    const lines = [];
    const indent = getIndent(depth);
    lines.push(`${indent}<BuildableProductRunnable`);
    if (runnable.runnableDebuggingMode !== undefined) {
        lines.push(`${indent}   runnableDebuggingMode = "${runnable.runnableDebuggingMode}">`);
    }
    else {
        lines[lines.length - 1] += ">";
    }
...
function buildRemoteRunnable(runnable, depth) {
    const lines = [];
    const indent = getIndent(depth);
    const attrs = [];
    if (runnable.runnableDebuggingMode !== undefined) {
...
function buildLocationScenarioReference(ref, depth) {
    const lines = [];
    const indent = getIndent(depth);
...
function buildStoreKitConfigurationFileReference(ref, depth) {
    const lines = [];
    const indent = getIndent(depth);
...
function buildProfileAction(action, depth) {
    const lines = [];
    const indent = getIndent(depth);
    const attrs = [];
    if (action.askForAppToLaunch !== undefined) {
...
function buildAnalyzeAction(action, depth) {
    const lines = [];
    const indent = getIndent(depth);
...
function buildArchiveAction(action, depth) {
    const lines = [];
    const indent = getIndent(depth);
    const attrs = [];
    if (action.buildConfiguration !== undefined) {
    ...
    if (action.revealArchiveInOrganizer !== undefined) {
        attrs.push(`revealArchiveInOrganizer = "${boolToString(action.revealArchiveInOrganizer)}"`);
    }
...
function buildCommandLineArguments(args, depth) {
    const lines = [];
    const indent = getIndent(depth);
    ...
    for (const arg of args) {
        ...
        if (arg.isEnabled !== undefined) {
            lines.push(`${getIndent(depth + 1)}   isEnabled = "${boolToString(arg.isEnabled)}">`);
        }
        else {
            lines[lines.length - 1] += ">";
        }
...
function buildEnvironmentVariables(vars, depth) {
    const lines = [];
    const indent = getIndent(depth);
    ...
    for (const v of vars) {
        ...
        if (v.isEnabled !== undefined) {
            lines.push(`${getIndent(depth + 1)}   isEnabled = "${boolToString(v.isEnabled)}">`);
        }
        else {
            lines[lines.length - 1] += ">";
        }
